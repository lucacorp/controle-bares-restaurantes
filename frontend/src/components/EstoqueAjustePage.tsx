// src/components/EstoqueAjustePage.tsx
...
// REMOVA ESTA INTERFACE (não é mais necessária se não usar useOutletContext):
// interface OutletContext {
//   productId: number;
// }

// ADICIONE a interface para as props
interface EstoqueAjustePageProps {
  productId: number;
}

// ATUALIZE a assinatura da função para receber 'productId' como prop
export default function EstoqueAjustePage({ productId }: EstoqueAjustePageProps) {
  // REMOVA ESTA LINHA: const { productId } = useOutletContext<OutletContext>();

  console.log("EstoqueAjustePage recebeu productId VIA PROP:", productId); // Verifique este log
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post("/movimentacoes-estoque", payload); // Confirme que o endpoint está correto com /api
